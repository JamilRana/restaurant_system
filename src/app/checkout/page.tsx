...
const CheckoutPage = () => {
  ...
  }, [session, status, deliveryMode, setDeliveryMode, guestName, guestEmail, router]);
  ...
  useEffect(() => {
    ...
    const fetchDeliveryFee = async () => {
      try {
        const res = await fetch(`/api/delivery-fee?postcode=${encodeURIComponent(postcode)}`);
        const data = await res.json();
        setDeliveryFee(res.ok && typeof data.deliveryFee === "number" ? data.deliveryFee : 0);
      } catch (err) {
  ...
  // Recalculate total
  useEffect(() => {
    const itemTotal = basketItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
  ...
  useEffect(() => {
    ...
    const collectionTime = restInfo.restaurant.collectionTime?.trim() || "18:00-22:00";
 ...
 const handleApplyPromo = async () => {
  ...
  if (deliveryMode === "delivery") {
    if (!postcode || !address) {
      setErrors(prev => ({ ...prev, address: "Please enter delivery address" }));
      hasError = true;
    }
    if (!session) {
      setErrors(prev => ({ ...prev, address: "Login required for delivery" }));
      hasError = true;
    }
  }
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <div className="flex-1 space-y-6">
          {/* Time Slot */}
          <div>
            <label className="block mb-2 font-medium text-sm">Choose your time slot</label>
            <select
              value={timeSlot}
              onChange={(e) => {setTimeSlot(e.target.value);setErrors({...errors, timeSlot: ""});}}
            ...
            >
              ...
              {restInfo.restaurant ? (() => {
                const collectionTime = restInfo.restaurant.collectionTime?.trim() || "18:00-22:00";
                const [startStr, endStr] = collectionTime.split("-").map(s => s.trim());
                if (!startStr || !endStr) return <option disabled>Invalid time format</option>;
                const parseHour = (time: string) => {
                  const match = time.match(/^(\d{1,2}):(\d{2})$/);
                  return match ? parseInt(match[1], 10) : null;
                };
          ...
          {/* Payment Method */}
          <div>
            <label className="block mb-2 font-medium text-sm">Payment Information</label>
...
{/*
          ...
          <div className="flex gap-4">
            ...
            <Link href="/" className="w-full bg-orange-500 text-center text-white font-bold rounded py-2">
              ORDER MORE
            </Link>
          </div>
        </div>
