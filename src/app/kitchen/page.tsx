...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`px-2 py-1 rounded text-xs font-semibold capitalize ${styles[status as keyof typeof styles] || "bg-gray-100 text-gray-800"}`}
    >
      {status}
    </span>
  );
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white p-4 rounded-lg shadow mb-6 flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            <SearchBar onSearch={setSearch} placeholder="Search by ID or name..." />
          </div>
          <select
            value={statusFilter}
            onChange={(e) => {
...
{order.items.map(item => {
        ...
        <div className="mt-1 inline-flex items-center gap-1 text-xs bg-red-600 text-white px-2 py-0.5 rounded-full">
          🔁 Added Later
                        ...
                        {order.items && order.items.length > 3 && (
                          <div className="text-gray-500">+{order.items.length - 3} more</div>
                        )}
                      </td>
                      <td className="px-4 py-3 text-sm capitalize">{order.deliveryType.toLowerCase()}</td>
  ...
  >
    <Image src="/icons/details.png" alt="Details" width={16} height={16} />
    Details
  </button>

  {/* ✅ Show Staff Name */}
  <div className="text-xs text-gray-600 mt-1">
    By: {order.createdBy?.staff?.name || order.createdBy?.email.split('@')[0] || "Unknown"}
  </div>
